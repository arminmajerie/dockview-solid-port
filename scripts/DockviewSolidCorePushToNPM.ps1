...
$corePackageDirectory = Join-Path $workspaceRoot 'packages\dockview-core'
$corePackageJsonPath = Join-Path $corePackageDirectory 'package.json'
$dependencyName = '@arminmajerie/dockview-core'
...
try {
  ...
  if (-not $SkipInstall) {
    Step "Installing dependencies in $corePackageDirectory"
    & npm install
    if ($LASTEXITCODE -ne 0) {
